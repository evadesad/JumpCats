...
function render() {
  ...
  app.innerHTML = `
    <main>
      ...
      <section class="controls"><label>Dropzone <select id="dropzone">${state.dropzones.map(d => `<option value="${d.id}" ${d.id === state.selected.id ? 'selected' : ''}>${escapeHtml(d.name)}${d.place ? ` — ${escapeHtml(d.place)}` : ''}</option>`).join('')}</select></label><button id="refresh" class="primary">↻ Refresh forecast</button><label style="margin-left:auto;color:var(--ink)"><input type="checkbox" checked disabled style="accent-color:var(--teal)" /> Should I go to dropzone? <b style="color:var(--teal)">Always.</b></label><span class="source">${state.source}</span></section>
